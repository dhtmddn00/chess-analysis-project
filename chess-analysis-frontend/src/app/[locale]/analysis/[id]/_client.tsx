      ...
      <div className="bg-white shadow-sm chess-hero">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="pt-4 pb-1 flex items-center justify-between">
            ...
            {/* P2-5: Reanalyze button */}
            <button
              type="button"
              onClick={() => {
                const params = new URLSearchParams();
                params.set('username', result.username);
                if (result.platform) params.set('platform', result.platform);
                router.push(`/analyze?${params.toString()}`);
              }}
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-zinc-500 hover:text-zinc-900 border border-zinc-200 rounded-lg px-3 py-1.5 hover:bg-zinc-50 transition-colors"
            >
