...
export function JobProgress({ status, className = '' }: JobProgressProps) {
  ...
  const getStatusText = () => {
    switch (status.status) {
      ...
      case 'in_progress': return tCommon('inProgress');
      case 'completed': return tCommon('completed');
      case 'failed': return tCommon('failed');
  ...
  return (
    <div className={`bg-white rounded-xl shadow-lg p-6 ${className}`}>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-3">
          ...
          {status.analysisVersion && (
            <div className="text-xs text-gray-500">v{status.analysisVersion}</div>
          )}
        </div>
        ...
        {status.etaRemainingSec && status.status === 'in_progress' && (
          <div className="flex items-center space-x-1 text-sm text-gray-600">
            ...
            <span>{t('etaMinutes', { minutes: String(Math.ceil(status.etaRemainingSec / 60)) })}</span>
          </div>
        )}
      </div>
      ...
      {status.error && (
        <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
          <div className="flex items-center space-x-2">
            ...
            <span className="font-medium text-red-800">{t('errorTitle')}</span>
          </div>
      ...
      <div className="space-y-4">
        <h3 className="font-semibold text-gray-900 mb-3">{t('sections')}</h3>
                ...
                {section.ready && section.data && (
                  <div className="text-sm text-gray-600">
                    ...
                    {section.id === 'endgame' && section.data.score !== undefined && (
                      <span>{t('scorePoints', { score: String(section.data.score) })}</span>
                    )}
                    {section.id === 'time_mgmt' && section.data.score !== undefined && (
                      <span>{t('scorePoints', { score: String(section.data.score) })}</span>
                    )}
                    {section.id === 'swing_moments' && section.data.top && (
                      <span>{t('countItems', { count: String(section.data.top.length) })}</span>
                    )}
                  </div>
                )}
      ...
      {status.status === 'completed' && (
        <div className="mt-6 p-4 bg-green-50 border border-green-200 rounded-lg">
          <div className="flex items-center space-x-2">
            <CheckCircle className="w-5 h-5 text-green-600" />
            <span className="font-medium text-green-800">{t('completedTitle')}</span>
          </div>
          <p className="text-sm text-green-700 mt-1">{t('completedDesc')}</p>
        </div>
      )}
    </div>
  );
