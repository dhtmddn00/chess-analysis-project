...
export function PlayerSummaryCard({ 
  ...
  const getCohortBadgeColor = (band: string) => {
    if (band.includes('super_elite') || band.includes('2900')) {
      return 'bg-purple-100 text-purple-800 border-purple-200';
    } else if (band.includes('grandmaster') || band.includes('2600')) {
      return 'bg-yellow-100 text-yellow-800 border-yellow-200';
    } else if (band.includes('master') || band.includes('2200')) {
      ...
      <div className="bg-gradient-to-r from-blue-600 to-purple-600 text-white p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-sm text-blue-200 mb-1">최고 레이팅</div>
            <div className="text-2xl font-bold">
              {Math.max(...Object.values(player.ratings))}
            </div>
            {cache_status === 'cached' && (
              <div className="text-xs text-blue-200 mt-1">캐시됨</div>
            )}
          </div>
        </div>
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-semibold text-gray-900">전체 전적</h3>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="text-center p-3 bg-gray-50 rounded-lg">
              <div className="text-2xl font-bold text-green-600">
                {(player.record_all.winrate * 100).toFixed(1)}%
              </div>
          ...
          <div className="flex justify-between text-sm">
            <span className="text-green-600">승: {player.record_all.win}</span>
            <span className="text-yellow-600">무: {player.record_all.draw}</span>
            <span className="text-red-600">패: {player.record_all.loss}</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-semibold text-gray-900">주요 오프닝</h3>
          </div>
          
          <div className="space-y-2">
            <div>
              <div className="text-sm font-medium text-gray-700 mb-1">백 (White)</div>
              {openings.white_top.slice(0, 2).map((opening, index) => (
                <div key={index} className="flex justify-between items-center text-sm">
                  <span className="text-gray-600">
                    {opening.eco} {opening.name.slice(0, 20)}...
                  </span>
                  <span className="text-green-600 font-medium">
                    {(opening.winrate * 100).toFixed(0)}%
                  </span>
                </div>
              ))}
            </div>
            ...
            <div>
              <div className="text-sm font-medium text-gray-700 mb-1">흑 (Black)</div>
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-2">
            ...
            <h3 className="font-semibold text-gray-900">최근 게임</h3>
          </div>
