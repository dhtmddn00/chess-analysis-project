...
from .simple_tactical_detector import analyze_simple_tactical_opportunities, create_simple_tactical_evidence, analyze_tactical_opportunity_usage
...
class StockfishEngine:
    ...
    def __init__(self):
        ...
        # self.tactical_detector = TacticalDetector()  # 새로운 간단한 탐지기 사용
    ...
    async def analyze_move(
        ...
        # 중요한 수에서만 전술 분석 (성능 고려)
        if centipawn_loss > 30 or is_capture or is_check:
            try:
                # 전술 기회 활용도 분석
                tactical_usage = analyze_tactical_opportunity_usage(
                    board_before, move, best_move
                )
                
                # 전술 정보를 기존 형식으로 변환 (레거시 호환성)
                if tactical_usage['tactic_found']:
                    tactical_opportunities.append({
                        'pattern': tactical_usage['tactic_type_played'],
                        'value_gain': abs(centipawn_loss),
                        'difficulty': 2,
                        'description': f"전술 활용: {tactical_usage['tactic_type_played']}",
                        'found': True
                    })
                    tactical_motifs.append(tactical_usage['tactic_type_played'])
                
                if tactical_usage['tactic_missed']:
                    tactical_opportunities.append({
                        'pattern': tactical_usage['tactic_type_missed'],  
                        'value_gain': abs(centipawn_loss),
                        'difficulty': 2,
                        'description': f"놓친 전술: {tactical_usage['tactic_type_missed']}",
                        'found': False
                    })
            
            except Exception as e:
                logger.debug(f"전술 탐지 실패 (ply {ply}): {e}")
        ...
        return MoveAnalysis(
            ...
            tactical_usage=tactical_usage  # 새로운 전술 활용도 정보
        )
